...
### Docker Extract Volume Sync Always Runs (2025-08-15)
- Ensured the helper container + rsync block in `internal/steps.go/ProcessDockerExtractVolumeStep` runs unconditionally after a successful `docker volume create`.
- Previously, rsync only ran inside the `if err != nil` branch, so a successful volume create skipped syncing.
- Now we return early on create error and always run the container with mounts: `/original`, `/solution1..4`, `/golden`, then install rsync and sync:
  - `/original` <- `AppFolder`
  - `/solution1..4` <- `/original`
  - `/golden` <- `/original`
- Build verified with `go build ./...`.
- Aligns with golden container mount path set to `tasks.settings.app_folder` in `internal/process_docker_pool.go`.
