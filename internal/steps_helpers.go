...
func generatedStepsExist(db *sql.DB, parentStepID int) (bool, error) {
	var existAndAreValid bool
	// Check if steps were generated by the parent, and if they all completed successfully.
	// If a step was generated but is not in a 'success' state (e.g., it's pending, running, or failed),
	// we treat it as not existing so it can be regenerated.
	query := `
		SELECT
			COUNT(*) > 0 AND
			COUNT(*) FILTER (WHERE results->>'result' != 'success' OR results IS NULL) = 0
		FROM steps
		WHERE (settings->'docker_shell'->>'generated_by')::int = $1`
	err := db.QueryRow(query, parentStepID).Scan(&existAndAreValid)
	if err != nil {
		return false, fmt.Errorf("failed to query for generated steps: %w", err)
	}
	return existAndAreValid, nil
}
