...
func GetGeneratedSteps(db *sql.DB, generatedByStepID int) ([]Step, error) {
	query := `
		SELECT s.id, s.task_id, s.title, s.settings, s.results, s.created_at, s.updated_at, t.local_path
		FROM steps s
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.TaskID, &s.Title, &s.Settings, &results, &s.CreatedAt, &s.UpdatedAt, &s.LocalPath); err != nil {
...
// Define a new struct for solution assignments
...
type RubricShellConfig struct {
	...
	Assignments      []SolutionAssignment `json:"assignments,omitempty"`  
...
type RubricSetConfig struct {
	...
	AssignContainers map[string]string `json:"assign_containers" db:"assign_containers"`
