...
func GetGeneratedSteps(db *sql.DB, generatedByStepID int) ([]Step, error) {
	query := `
		SELECT s.id, s.task_id, s.title, s.settings, s.results, s.created_at, s.updated_at, t.base_path
		FROM steps s
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.TaskID, &s.Title, &s.Settings, &results, &s.CreatedAt, &s.UpdatedAt, &s.BasePath); err != nil {
