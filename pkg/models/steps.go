...
func StoreStepResult(db *sql.DB, stepID int, result map[string]interface{}) error {
    ...
    if err := db.QueryRow(`SELECT results FROM steps WHERE id = $1`, stepID).Scan(&existing); err != nil {
        return fmt.Errorf("failed to read existing results for step %d: %w", stepID, err)
    }

    // Unmarshal existing results into a map if present
    merged := make(map[string]interface{})
    if existing.Valid && existing.String != "" {
        var old map[string]interface{}
        if err := json.Unmarshal([]byte(existing.String), &old); err != nil {
            // If corrupt, log and proceed with new results only
            StepLogger.Printf("[WARN] Existing results JSON invalid for step %d: %v; overwriting with new results", stepID, err)
        } else {
            for k, v := range old {
                merged[k] = v
            }
    ...
    // Merge: new keys overwrite old keys, others preserved
    for k, v := range result {
        merged[k] = v
    }

    resJSON, _ := json.Marshal(merged)
